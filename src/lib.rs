    ...
    for i in 0..ny / 2 {
        ...
        for j in 0..simd_nx {
            unsafe {
                ...
                let vl1 = f64x4::new(vp1.extract(3), v1.extract(0), v1.extract(1), v1.extract(2));
                let vl2 = f64x4::new(vp2.extract(3), v2.extract(0), v2.extract(1), v2.extract(2));
                let vr1 = f64x4::new(v1.extract(1), v1.extract(2), v1.extract(3), vn1.extract(0));
                let vr2 = f64x4::new(v2.extract(1), v2.extract(2), v2.extract(3), vn2.extract(0));
...
mod test {
    ...
    fn test_dim(ny: usize, nx: usize) {
        ...
        if err > 1e-10 {
            println!("{}\n", w);
            println!("{}\n", w_ref);
            println!("{}", d.mapv(|x| {if x.abs() < 1e-14 { 0. } else { x }}));
